export default class AtoriaItemSheetV1 extends ItemSheet {
  ...
  async getData() {
    ...
    const item_types_with_associated_skill = [
      "feature",
      "action",
      "opportunity",
    ];
    ...
    if (item_types_with_actable_modifiers.includes(this.item.type)) {
      const want_technique =
        this.item.type === "weapon" ||
        (this.item.type === "action" && !this.item.system.is_magic);
      const want_incantatory =
        this.item.type === "spell" ||
        (this.item.type === "action" && this.item.system.is_magic);
      context.available_actable_modifiers = this.actor?.getActableModifierList(
        want_technique,
        want_incantatory,
      );
    }
    const item_types_with_saves_asked = [
      "action",
      "opportunity",
      "spell",
      "technique",
  ...
  activateListeners(html) {
    ...
    html.on("click", ".expand-control", (event) => {
      ...
      if ($control_element.hasClass("expanded"))
    ...
    for (const id in this.expanded_section) {
      html
        .find(`.expand-control[data-expand-id="${this.expanded_section[id]}"]`)
        .each((idx, element) => {
